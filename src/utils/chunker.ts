...
export async function combineChunks(
  ...
  for (let i = 1; i < values.length - 1; i += 1) {
    if (values[i - 1].length !== values[i].length) {
      console.warn(
        "@tealbase/ssr: Possible issue reconstructing chunked cookies. Oddly sized cookie chunks detected. Please check your integration with tealbase for bugs. This can cause your users to loose the session.",
      );
      break;
    }
  }
