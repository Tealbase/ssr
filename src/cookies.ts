...
export function decodeBase64Cookie(value: string) {
  try {
    ...
    console.warn(
      "@tealbase/ssr: Detected stale cookie data that does not decode to a UTF-8 string. Please check your integration with tealbase for bugs. This can cause your users to loose session access.",
    );
    return null;
  }
}
...
export function decodeCookie(chunkedCookie: string) {
  ...
  if (chunkedCookie.startsWith(BASE64_PREFIX)) {
    ...
    if (match[2].length !== expectedLength) {
      console.warn(
        "@tealbase/ssr: Detected stale cookie data. Please check your integration with tealbase for bugs. This can cause your users to loose the session.",
      );
    }
  ...
  if (cookies) {
    if ("get" in cookies) {
      ...
      if ("set" in cookies && "remove" in cookies) {
        ...
        setAll = async () => {
          console.warn(
            "@tealbase/ssr: createServerClient was configured without set and remove cookie methods, but the client needs to set cookies. This can lead to issues such as random logouts, early session termination or increased token refresh requests. If in NextJS, check your middleware.ts file, route handlers and server actions for correctness. Consider switching to the getAll and setAll cookie methods instead of get, set and remove which are deprecated and can be difficult to use correctly.",
          );
        };
      } else {
        throw new Error(
          "@tealbase/ssr: createBrowserClient requires configuring a getAll and setAll cookie method (deprecated: alternatively both get, set and remove can be used)",
        );
      }
      ...
      if ("setAll" in cookies) {
        ...
        setAll = async () => {
          console.warn(
            "@tealbase/ssr: createServerClient was configured without the setAll cookie method, but the client needs to set cookies. This can lead to issues such as random logouts, early session termination or increased token refresh requests. If in NextJS, check your middleware.ts file, route handlers and server actions for correctness.",
          );
        };
      } else {
        throw new Error(
          "@tealbase/ssr: createBrowserClient requires configuring both getAll and setAll cookie methods (deprecated: alternatively both get, set and remove can be used)",
        );
      }
    } else {
      // neither get nor getAll is present on cookies, only will occur if pure JavaScript is used, but cookies is an object
      throw new Error(
        `@tealbase/ssr: ${isServerClient ? "createServerClient" : "createBrowserClient"} requires configuring getAll and setAll cookie methods (deprecated: alternatively use get, set and remove).${isBrowser() ? " As this is called in a browser runtime, consider removing the cookies option object to use the document.cookie API automatically." : ""}`,
      );
    }
    ...
    throw new Error(
      "@tealbase/ssr: createServerClient must be initialized with cookie options that specify getAll and setAll functions (deprecated, not recommended: alternatively use get, set and remove)",
    );
  } else {
    ...
    setAll = () => {
      throw new Error(
        "@tealbase/ssr: createBrowserClient in non-browser runtimes (including Next.js pre-rendering mode) was not initialized cookie options that specify getAll and setAll functions (deprecated: alternatively use get, set and remove), but they were needed",
      );
    };
  }
